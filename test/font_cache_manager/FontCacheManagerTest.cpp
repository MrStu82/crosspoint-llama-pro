...
TEST(FontCacheManagerTest, MissingBuiltInStyleKeepsFamilyFallbackSemantics) {
  EpdFontData regularData{reinterpret_cast<const void*>(1)};
  EpdFontData boldData{reinterpret_cast<const void*>(1)};
  const EpdFontFamily family(&regularData, &boldData, nullptr, nullptr);
  const std::map<int, EpdFontFamily> builtinFonts{{5, family}};
  const std::map<int, SdCardFont*> noSdFonts;
  FontDecompressor decompressor;
  FontCacheManager manager(builtinFonts, noSdFonts);
  ...
  auto scope = manager.createPrewarmScope();
  manager.recordText("R", 5, EpdFontFamily::ITALIC);
  manager.recordText("B", 5, EpdFontFamily::BOLD_ITALIC);
  scope.endScanAndPrewarm();
  ...
  const auto* regularCall = first.fontData == &regularData ? &first : &second;
  const auto* boldCall = first.fontData == &boldData ? &first : &second;
  EXPECT_STREQ("R", regularCall->text);
  EXPECT_STREQ("B", boldCall->text);
}
...
TEST(FontCacheManagerTest, CacheReleaseAndScopeCleanupKeepTheirDistinctSemantics) {
  SdCardFont readerFont;
  SdCardFont fallbackFont;
  const std::map<int, EpdFontFamily> noBuiltinFonts;
  const std::map<int, SdCardFont*> sdFonts{{-17, &readerFont}, {23, &fallbackFont}};
  ...
  EXPECT_EQ(0, readerFont.clearCacheCount);
  EXPECT_EQ(0, fallbackFont.clearCacheCount);
  ...
  // Scope construction and destruction clear transient caches, while the
  // heap-critical resident-cache release remains an explicit operation.
  EXPECT_EQ(3, decompressor.clearCacheCount);
  EXPECT_EQ(2, readerFont.clearCacheCount);
  EXPECT_EQ(2, fallbackFont.clearCacheCount);
  EXPECT_EQ(1, readerFont.releaseResidentCachesCount);
  EXPECT_EQ(1, fallbackFont.releaseResidentCachesCount);
