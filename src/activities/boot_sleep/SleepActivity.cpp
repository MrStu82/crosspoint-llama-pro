...
void SleepActivity::renderCustomSleepScreen() const {
  // Image decoders need a contiguous working buffer; SD glyph caches are
  // rebuildable and must not crowd it out.
  releaseSdFontCachesForDecode(renderer);

  HalFile file;
  // A pinned image is an explicit user choice and wins over rotation. If it was
  // removed or became corrupt, continue through the normal fallback chain.
  ...
  // A root sleep.bmp takes priority over rotating directories.
...
// because that path remains pending hardware validation. A real full-flash
// clear is preferred over trusting an unvalidated differential refresh.
