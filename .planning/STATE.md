...
Phase: 11 of 12 — Job Phase 4 loot-box implementation, gate pending (Milestone 3)
Plan: Job Phase 4 implementation is code-complete at the pre-gate checkpoint: four Common/Uncommon/Rare/Legendary box tiers, exact 50/30/15/5 selector, one compact table per tier reusing `ITEM_DEFS`/`BUFF_DEFS`/`SKILL_DEFS`, four visible `ItemDef` records occupying one logical shared-loot slot, and direct inventory opening with no safe-room gate. `SPONSOR_DEFS` remains presentation/per-floor state rather than being misused as durable Legendary loot. Deterministic host harness covers exact tier boundaries, table validity/exclusions, and the real shared-loot distribution. Clean-tree Trantor firmware/full-host gate is the next action; no image ships at this intermediate gate.
...
Stopped at: **Job Phase 4 loot-box implementation code-complete, pre-gate.** Four tiers/tables and deterministic harness are in the working tree; commit-first clean Trantor build is next.
Next actionable item: Commit the exact Phase 4 tree, run the standalone real-path loot-box harness plus full host gtest and `pio run -e x4pro` on Trantor, then record/report the phase gate. Continue to the next parent-authorised firmware phase only after that gate.
Resume file: `docs/world-dungeon-loot-box-design.md` and `test/loot_box/LootBoxEligibilityHarness.cpp`
