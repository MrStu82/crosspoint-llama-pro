...
void ChapterHtmlSlimParser::applyTextDecorationToEntry(StyleStackEntry& entry, const CssStyle& css) {
  ...
  if (css.hasBackgroundBlack()) {
    entry.hasBackgroundBlack = true;
    entry.backgroundBlack = css.backgroundBlack;
  }
}
...
void ChapterHtmlSlimParser::updateEffectiveInlineStyle() {
  ...
  for (const auto& entry : inlineStyleStack) {
    ...
    if (entry.hasBackgroundBlack) effectiveBackgroundBlack = entry.backgroundBlack;
  }
...
void ChapterHtmlSlimParser::startNewTextBlock(const BlockStyle& blockStyle) {
  ...
  // block is flushed so the chapter starts on a fresh page.
  flushPendingAnchor();
  currentTextBlock.reset(new ParsedText(extraParagraphSpacing, hyphenationEnabled, focusReadingEnabled, guideReadingEnabled,
                                  forceParagraphIndents, blockStyle));
...
void XMLCALL ChapterHtmlSlimParser::startElement(void* userData, const XML_Char* name, const XML_Char** atts) {
  ...
  if (self->tableDepth == 1 && (strcmp(name, "td") == 0 || strcmp(name, "th") == 0)) {
    ...
    self->currentTextBlock = makeUniqueNoThrow<ParsedText>(
        self->extraParagraphSpacing, self->hyphenationEnabled, self->focusReadingEnabled,
        self->guideReadingEnabled, self->forceParagraphIndents, tableCellBlockStyle);
  ...
  if (matches(name, HEADER_TAGS, std::size(HEADER_TAGS))) {
    ...
    if (strcmp(name, "br") == 0) {
      ...
      if (strcmp(name, "li") == 0) {
        self->currentTextBlock->addWord("\xe2\x80\xa2", EpdFontFamily::REGULAR, false, false, self->visibleTextOffset, self->effectiveBackgroundBlack);
    ...
    if (cssStyle.hasFontWeight() || cssStyle.hasFontStyle() || cssStyle.hasTextDecoration() ||
        cssStyle.hasDirection() || cssStyle.hasVerticalAlign() || cssStyle.hasBackgroundBlack() ||
...
void XMLCALL ChapterHtmlSlimParser::characterData(void* userData, const XML_Char* s, const int len) {
  ...
  if (!self->currentTextBlock) {
    ...
    self->currentTextBlock = makeUniqueNoThrow<ParsedText>(
        self->extraParagraphSpacing, self->hyphenationEnabled, self->focusReadingEnabled,
        self->guideReadingEnabled, self->forceParagraphIndents, flowStyle);
    if (!self->currentTextBlock) {
