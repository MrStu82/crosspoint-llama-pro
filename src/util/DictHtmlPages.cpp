...
// Per-page retention uses the lower, independently testable thresholds in
// DictHtmlPageBudget: the parser's working set is already live at that point,
// so reapplying this entry gate would reject a definition's first valid page.
...
bool buildDictionaryHtmlPages(GfxRenderer& renderer, const std::string& definition, const uint16_t viewportWidth,
  ...
  pagesOut.reserve(DictHtmlPageBudget::MAX_PAGES);
  ...
  size_t retainedElements = 0;
  {
    ...
    auto parser = makeUniqueNoThrow<ChapterHtmlSlimParser>(
        ...
        SETTINGS.hyphenationEnabled, SETTINGS.focusReadingEnabled, SETTINGS.guideReadingEnabled,
        ...
        [&pagesOut, &resourceLimitHit, &retainedElements, &limitReason](std::unique_ptr<Page> page, uint16_t, uint16_t,
          ...
          limitReason = DictHtmlPageBudget::limitName(DictHtmlPageBudget::retainedPageLimit(
              pagesOut.size(), retainedElements, pageElements, ESP.getFreeHeap(), ESP.getMaxAllocHeap()));
